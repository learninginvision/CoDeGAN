...
class Inception:
    def __init__(self):
        ...
        self.inception_input = tf.placeholder(
        ...
        self.inception_scores = tfgan.eval.classifier_score_from_logits(
            self.fake_activations_ph
        )
    ...
    def _preprocess_images(self):
        resized_images = tf.image.resize_bilinear(self.images_ph, INCEPTION_SHAPE)
        self.inception_input_temp = (resized_images - 127.5) / 127.5

    def _import_inception_graph(self):
        with tf.gfile.FastGFile(
            os.path.join(MODEL_DIR, INCEPTION_GRAPH_NAME), "rb"
        ) as f:
            ...
            self.preds = tf.exp(self.logits1) / tf.reduce_sum(
                tf.exp(self.logits1), 1, keepdims=True
            )
    ...
    def _IS(self, x, sess):
        ...
        for batch_idx, (batch_start, batch_end) in enumerate(batches):
            ...
            a, b = sess.run(
                [self.inception_features, self.preds], {self.inception_input: img}
            )
            activation.append(a)
            logist.append(b)
            scores = []
            if k_step % 10 == 9:
                ...
                score = sess.run(
                    self.inception_scores, {self.fake_activations_ph: logist}
                )
    ...
    def _fid(self, x, r_mu, r_sigma, sess):
        ...
        for batch_idx, (batch_start, batch_end) in enumerate(batches):
            ...
            a, b = sess.run(
                [self.inception_features, self.logits], {self.inception_input: x_batch}
            )
            f_act.append(a)
            logist.append(b)
            if k_step % 10 == 9:
            ...
            if t_step % 100 == 99:
                ...
                print(
                    "IS_mean: %.4f" % np.mean(t_score), "IS_std: %.4f" % np.std(t_score)
                )
                f_act = []
            t_step += 1
            k_step += 1
...
def load_Img(imgDir, imgFoldName):
    ...
    imgNum = 50000
    data = np.empty((imgNum, 128, 128, 3), dtype="float32")
    for i in range(imgNum):
        img = Image.open(imgDir + imgFoldName + "/" + imgs[i])
        arr = np.asarray(img, dtype="float32")
        data[i, :, :, :] = arr[45 : 45 + 128, 25 : 25 + 128, :]
    return data


with tf.device("/gpu:0"):
    x_train = np.load("./generated_imgs.npy")
    print(x_train.max(), x_train.min())
    print(x_train.shape)
    np.random.shuffle(x_train)
    r_cifar_act = np.load("fid_stats_cifar10_train.npz")
    r_mu = r_cifar_act["mu"]
